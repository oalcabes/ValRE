...
#Changes in 0.5a: This is a "branch" to the original code. This version
#requires the user to input a threshold, or will default to >100 MeV, 1 pfu.
#This version will only calculate info for one threshold and the output files
#contain the threshold in the filenames.
#Changes in 0.5: Added ability to download and plot SOHO/ERNE data for the
#>10 MeV threshold calculations.
...
def all_program_info(): #only for documentation purposes
    ...
    SEP event values are calculated for ONE threshold definition defined by
    the user through the --Threshold input variable. The default is:
...
def get_fluence_spectrum(experiment, flux_type, model_name, energy_threshold,
    ...
    if save_file:
        ...
        foutname = outpath + '/fluence_' + str(experiment) + '_' \
                    + str(flux_type) + '_' + 'gt' + str(int(energy_threshold)) \
                    + '_' + str(flux_threshold) + 'pfu_' + str(year) + '_' \
                    + str(month) + '_' + str(day) +'.csv'
        if experiment == 'user' and model_name != '':
            foutname = outpath + '/fluence_' + model_name + '_' \
                        + str(flux_type) + '_' + 'gt' \
                        + str(int(energy_threshold)) + '_' \
                        + str(flux_threshold) + 'pfu_' + str(year) + '_' \
                        + str(month) + '_' + str(day) +'.csv'
...
def save_integral_fluxes_to_file(experiment, flux_type, model_name,
        energy_thresholds, flux_thresholds, crossing_time, dates,
        integral_fluxes):
    """
    """
    nthresh = len(energy_thresholds)
    ...
    foutname = outpath + '/integral_fluxes_' + experiment + '_' \
                 + flux_type + '_' + 'gt' + str(int(energy_thresholds[0])) \
...
def print_values_to_file(experiment, flux_type, model_name, energy_thresholds,
    ...
    foutname = outpath + '/sep_values_' + experiment + '_' \
                + flux_type + '_' + 'gt' + str(int(energy_thresholds[0])) \
                + '_' + str(int(flux_thresholds[0])) + 'pfu_' + str(year) + '_' \
                + str(month) + '_' + str(day) +'.csv'
    if experiment == 'user' and model_name != '':
    ...
    for i in range(nthresh):
        if crossing_time[i] == 0: #no threshold crossed
            continue
        fout.write('>' + str(int(energy_thresholds[i])) + ',')
...
def run_all(str_startdate, str_enddate, experiment, flux_type, model_name,
    ...
    str_thresh = str_thresh.split(",")
    input_threshold = [int(str_thresh[0]), int(str_thresh[1])]
    ...
    energy_thresholds = [] #MeV; flux for particles of > this MeV
    flux_thresholds = [] #pfu; exceed this level of intensity
